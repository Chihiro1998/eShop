...
import { getTotalSales, getTotalCustomers, getSalesByMonth, getProfitByMonth } from "@/lib/actions/actions";
...
export default async function Home() {
  const { numberOfOrders, totalRevenue, totalProfit } = await getTotalSales();
  const totalCustomers = await getTotalCustomers();
  const salesByMonth = await getSalesByMonth();
  const profitByMonth = await getProfitByMonth();
